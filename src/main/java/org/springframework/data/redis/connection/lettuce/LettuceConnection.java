	...
	LettuceInvoker invoke() {
		...
		try {
			...
			if (isPipelined()) {
				...
				return new LettuceInvoker(getAsyncConnection(), (future, converter) -> {
			...
			return new LettuceInvoker(getAsyncConnection(),
					(future, converter) -> NullableResult.of(await(future)).convert(converter).get());
